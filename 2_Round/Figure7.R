...
################################################################################
## << Gross emissions per municipality in Brazil, 
###     and the ranking of the ten greatest emitter municipalities in 2019 >>
################################################################################
...
## 2 option
# Definir intervalo de colunas a serem processadas
#inicio_coluna <- 15
#fim_coluna <- 64

# Iterar sobre as colunas do intervalo e dividir cada valor por 1.000.000
#for (coluna in inicio_coluna:fim_coluna) {
#  dff9[[coluna]] <- as.numeric(dff9[[coluna]]) / 1000000
#}
...
dataset_final1 = left_join(df1, all_mun, by=c("CODIBGE"="code_muni"))

dataset_final3 = left_join(all_mun,df1, by=c("code_muni"="CODIBGE")) %>%
  drop_na()
...
coord_pontos2 <- dataset_final3 %>% 
  mutate(VALOR2 = VALOR) %>% 
  st_centroid()

coord_pontos333 <- rank %>% 
  mutate(VALOR2 = VALOR) %>% 
  st_centroid()
...
# Segregar pelos dois tipos
top10_both <- dataset_final1 %>%
  filter(`NIVEL 2` %in% c("Alterações de Uso do Solo", "Resíduos Florestais")) %>%
  group_by(`NIVEL 2`) %>%
  top_n(10, VALOR) %>%
  ungroup()

# Visualizar os top 10 de ambos os níveis
top10_both
...
#Juntar os top 20 confrome Nivel 2
top100 <- rbind(top10E, top10W) 
...
a<- rbind(top10E,top10W)

top10 <- dataset_final1 %>%
  #filter(`NIVEL 2` == "Resíduos Florestais")%>% 
  # selecionar o top 10
  top_n(10, VALOR)    
...
top10b <- top10%>% 
  mutate(`rank` = recode(`Nome_Município`,
                            ...
                            `Colniza` = "8",
                            `Itaituba` = "9",
                            `Novo Repartimento` = "10"))

top10$mun_and_state = paste(top10$name_muni,", ",top10$abbrev_state,sep = "")

top10b$mun_and_state = paste(top10b$rank," - ", top10b$name_muni,", ",top10b$abbrev_state,sep = "")
...
ggsave("output/Figure7_B.jpeg", plot = p1,dpi = 330)
...
windowsFonts(fonte.tt= windowsFont("TT Times New Roman"))
#windowsFonts(fonte= windowsFont("Arial"))
b<- ggplot() +
  geom_sf(data = dataset_final3, fill = "transparent", color=alpha("black",.25))+
  geom_sf(data=dataset_final3, aes(fill=VALOR), size=.125, color=alpha("orange",0.01))+
  scale_fill_distiller(palette = "Oranges",type="seq",trans = "reverse", name=expression(bold("Mt CO"["2"]*"e")))+
  theme_minimal()+
  ...
  geom_sf_text(data=ranks, aes(label=ranks), size=3)+
  theme(legend.background = element_blank())+
  theme(legend.title = element_text(color = "black", family = "fonte.tt", size=9))+
  theme(axis.title = element_text(color = "black",family = "fonte.tt", size=9))+
  theme(legend.text =  element_text(color = "black",family = "fonte.tt", size=9))+ # Aqui e a letra da legenda
...
ggsave("output/Figure8_AAAAAb.jpeg", plot = a,
       width = 18,
       height = 18,
       units = c("cm"),
       dpi = 330)
...
p<- ggpubr::ggarrange(a, p1,
                      ...
                      #align = "hv",# Align them both, horizontal and vertical
                      nrow = 2,
                      legend.grob = NULL)  # number of rows
...
ggarrange(a,p1 + rremove("x.text"), 
          #labels = c("A", "B"),
          ncol = 2, nrow = 2)


ggarrange(
  ggarrange(a, p1, nrow = 1,
            widths= c(1,.1),
            heights = c(2,1),# list of plots
            #widths = c(1, 1, 0), 
            legend = "none"),
  ggarrange(blank, plt.3, blank, nrow = 1,
...
combined_plot5 <- arrangeGrob(b, p1, ncol = 1, heights = unit(c(2, 1), "null"), widths = unit(1, "null"))

ggsave("output/Figure8-OK.jpeg", plot = combined_plot5, scale = 1,
       width = 18,
       height = 18,
       units = c("cm"),
       dpi = 330)
...
ggsave("Figure8_B.jpeg", plot = p1,dpi = 330)
...
a3<- ggplot() +
  ...
  scale_fill_distiller(palette = "Greens",type="seq", name="Mt Co2eq", breaks=seq(0,-40,-5))+
  theme_minimal()+
  theme(legend.position=c(.88,.85), legend.box = "horizontal",legend.justification = "center", legend.direction = "horizontal")+
  theme(legend.key = element_blank())+
  ylab("") + xlab(" ")+
...
ggsave("Figure8_AA.jpeg", plot = a3,scale = 1,
       width = 18,
       height = 18,
       units = c("cm"),
       dpi = 330)
...
# Plot 2
a2<- ggplot() +
  geom_sf(data=dataset_finalGROSS, aes(fill=VALOR), size=.125, color=alpha("gray",0.1))+
...
ggsave("Figure9B.jpeg", plot = p1, scale = 1,
       width = 18,
       height = 18,
       units = c("cm"),
       dpi = 330)
...
ggsave("Figure6.jpeg", plot = p, scale = 1,
       width = 12,
       height = 12,
       units = c("cm"),
       dpi = 330)
...
df <- reshape (dff9, varying = list(colnames(dff9[2:31])),
               times = names(dff9[2:31]),
               timevar = "ANO",
               direction = "long")
...
mutate(`NIVEL 2` = recode(`NIVEL 2`,
  ...
  ggplot(aes(x=ANO, y=VALOR, fill= factor(`NIVEL 2`, levels = c('Emissions by the burning of vegetation residuals',
                                                                'Emissions by land use change',
                                                                'Removals by secondary vegetation',
                                                                'Removals by other type of land use change',
                                                                'Removals in protected areas'))))+
  ...
  scale_x_discrete(breaks=seq(1990,2019,1))+
  #scale_x_continuous(limits=c(1990, 2020), breaks = c(1990,2000,2010,2020)) +
  scale_fill_manual(values=c("#fcd5b5","#c0504d","#92d050","#d7e4bd","#4f6228"))+
  scale_color_manual(labels = c('Emissions by the burning of vegetation residuals',
                                'Emissions by land use change',
                                'Removals by secondary vegetation',
                                'Removals by other type of land use change',
...
ggsave("Figure4.jpeg", plot = plot1,dpi = 330)
